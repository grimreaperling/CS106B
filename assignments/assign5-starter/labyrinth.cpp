 ...
 */
const string kYourName = "Herbert Yao";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "WWWSSSNNNEESSESNWSWN";
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    //string content = start->contents;
    //if (content != "") needs.remove(content);
    //if (needs.isEmpty()) return true;
    //if (moves.size() == 0) return false;
    int i = 0;
    needs.remove(start->contents);
    while (i < moves.size() && !needs.isEmpty()) {
        char cur = moves[i];
        if (cur == 'N') {
