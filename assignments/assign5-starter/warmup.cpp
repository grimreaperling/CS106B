...
void badDeallocate(ListNode* list) {
    while (list == nullptr) return;
    if (list->next != nullptr) {
        badDeallocate(list->next);
    }
    delete list;
}
...
PROVIDED_TEST("Segmentation fault: use of uninitialized values") {
    ListNode* q = new ListNode(4, nullptr);
    ListNode* p = new ListNode(5, q); // allocate new node, don't assign data or next field, leave uninitialized
